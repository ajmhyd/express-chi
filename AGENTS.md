...
### Imports
- Import from relative paths with `.ts`/`.tsx` extensions (e.g., `from "../types.ts"`)
...
- Return `null` on errors rather than throwing (see data/index.ts request function)
...
- Use `class` (not `className`) in Fresh routes, `className` in islands/components
